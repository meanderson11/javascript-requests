...
const sayHello = () => {
    axios
    .get('http://localhost:3000/say-hello').then((res) => {
...
const ohMy = () => {
    ...
    .then(res => {
      ...
      let oldOhMy =  document.createElement('p')
      oldOhMy.textContent = res.data[i]
      document.body.appendChild(oldOhMy);
...
const query = () => { 
    ...
    axios.get('http://localhost:3000/query-test?test=complete&maria=hello')
.then(res => {
...
const createFood = () => {
    let foodElem = document.getElementById('cheese')
    const body = {newFood: foodElem.value}
    axios.post('http://localhost:3000/food', body)
    .then(res => {
        console.log(res.data)

        let myCreateFood =  document.createElement('p')
      myCreateFood.textContent = res.data[res.data.length - 1]
      document.body.appendChild(myCreateFood);
    })
}
